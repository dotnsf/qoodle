...
- インターネットに接続可能なパソコン
  ...
  - この画面をパソコンで見ているのだとしたら、そのパソコンです。

  - 仮に主催するクイズ大会が全員が同じ部屋に集まって、１つの画面をプロジェクター等に投影して行うような場合であってもパソコン（と参加者のスマートフォン）にはインターネット接続が必要です。

  - （クイズがオンライン会議で提供される場合）オンライン会議を主催するためのアプリケーションがインストールされていること

    - 具体的には Zoom や Webex など、、、
...
- **画面右上のボタン** をクリックしてサインイン／サインアップ画面へ移動します：
...
- 登録したメールアドレスにメールが届くので、**24時間以内に確認リンクをクリック** することで正式な登録ができます。登録後はサインインが可能になります。

![qdle_user_ja_14.png](./doc/qdle_admin_ja_14.png)

- 改めて **サインイン** します。右上に名前が表示され、ここをクリックするとサインアウトします。また同画面には自分のアカウントで作成済みのクイズやクイズセットの一覧が表示されますが、サインアップ直後はどちらも空なので何も表示されません（クイズやクイズセットの作成方法は後述）：
...
クイズやクイズセットの具体的な編集内容については後述の **編集方法** 部を参照してください。
...
![qdle_admin_ja_06.png](./doc/qdle_admin_ja_06.png)
    ...
    - この公開クイズセットにはクイズが５問含まれているはずです。５問の内容は不定期に更新予定です。

    - クイズタイトル（test と表示されている部分）がクイズ表示／非表示のトグルになっています。１度クリックするとクイズは非表示になり、もう１度クリックすると再表示されます。

  - Qdle にサインインしていないと正しく表示されません

- オンラインでクイズ大会を開催する場合は **この画面をネットで参加者に共有** してください。

- オフラインで開催する場合は **この画面をプロジェクター等で参加者が見えるように投影** してください。

  
- **注意**

  - **この画面状態になった以降は主催者が画面をリロードするのは控えて** ください。主催者画面をリロードすると WebSocket にリセットがかかり、参加済みの参加者の情報が失われてしまいます。
...
- （参加者のつもりで）スマートフォンでクイズにアクセスします
  ...
  - スマートフォンを取り出し、**画面内の QR コードを読み取って参加者用 URL にアクセス** します

  - **Your name と書かれた箇所に自分のニックネームを入力** して **Start をタップ** します
...
![qdle_admin_ja_13.png](./doc/qdle_admin_ja_13.png)
    ...
    - 特定の参加者の絵をクリックすると **その絵だけを拡大** 表示することができます
...
![qdle_admin_ja_07.png](./doc/qdle_admin_ja_07.png)
  ...
  - クイズには **カテゴリー** が存在しており、各カテゴリーは以下のような形式で出題されます
  ...
  - 参加者は（制限時間がある場合は制限時間になる前に）答を推測して **スマホ画面に指で解答** します。
...
![qdle_admin_ja_08.png](./doc/qdle_admin_ja_08.png)
  ...
  - クイズ終了後、**解答画面タブを選択すると参加者全員の解答が表示** されています。

![qdle_admin_ja_09.png](./doc/qdle_admin_ja_09.png)

    - 主催者が **「正解」と判断した参加者については名前横の×印をクリック** します。フレームが緑色に変わると「正解した」と判断されます。もう一度クリックすると緑が元の色に戻り「正解ではなかった」と判断されます。

![qdle_admin_ja_10.png](./doc/qdle_admin_ja_10.png)

    - 全員ぶんの採点が終わったら **ランキング画面を表示** することもできます（必須ではないので、途中経過を見せたくない場合はそのまま次の問題へ移ってもかまいません）。ランキング画面タブを選択すると、参加者ごとの各問題への得点状況と現在の得点ランキングが上位から表示されます。
...
- 新規に作成する場合はこの画面下部までスクロールし、ID が空になっていることを確認して、カテゴリー、正解した場合の点数（通常は１）、解答時間（無指定時は30秒）、出題本文、コメント（解答終了後に表示される文字列で、ここに答を含めたコメント文を入力してください）、そして必要であれば画像 URL を入力します。最後に **保存** をクリックして保存します。

  - カテゴリーの種類と意味は上述のクイズカテゴリーを参照してください。

    - サウンドカテゴリーの場合は画像 URL 欄に mp3 ファイルの URL を指定することでクイズにサウンドを埋め込むことができます。

    - 動画カテゴリーの場合は画像 URL 欄に mp4 ファイルの URL か YouTube の URL を指定することでクイズに動画を埋め込むことができます。

  - 画像 URL を含める場合は著作権フリーの素材を使うか、自分の画像をアップロードして使うなど、<font color="red">著作権に留意</font>してください。

    - なお画像 URL に指定する URL は **公開されているもの** に限ります。特定のアカウントでログインしていないと参照できなかったり、特定の環境下でないと参照できなかったりする URL は指定できません。

![qdle_admin_ja_15.png](./doc/qdle_admin_ja_15.png)

- 既存のクイズを編集する場合は一覧から編集するクイズを選んで **EDIT ボタン** をクリックします。編集するクイズの ID や内容が編集フィールドに渡され、編集できるようになります。編集した内容を保存する場合は **保存** をクリックします。

![qdle_admin_ja_16.png](./doc/qdle_admin_ja_16.png)

- 既存クイズの **PREVIEW ボタン** を選択すると、クイズとして表示された時の様子をプレビューできます。思っていたような出題内容になっているかどうか確認できます：

![qdle_admin_ja_17.png](./doc/qdle_admin_ja_17.png)

- この画面から複数のクイズを選択（表の左のチェックボックスを選択）し、画面最下部の **クイズセット作成** を選択することで、選んだクイズを１つにまとめて新しいクイズセットを作成することができます。その場合はクイズセットのタイトルの入力が求められるので適宜指定してください（後で編集することもできます）。

  - クイズセットのログイン ID やパスワードの設定、自分が作成していないクイズをクイズセットに含める場合はこの操作からはできません。一度作成した後にクイズセットを編集して指定してください。
...
- サインイン直後の画面でクイズセットタブを選択します。現在までに作成したクイズセットの一覧が表示されます。
  ...
  - **一般ユーザーの場合、クイズセットは１題まで** 作成できます。**有償ユーザーの場合、クイズセット作成数に制限はありません** 。
...
- クイズセットの内容を編集する場合は一覧から **EDIT** を選択して、同様に編集／保存します。

![qdle_admin_ja_18.png](./doc/qdle_admin_ja_18.png)
...
- 主催したい **クイズセットを選択** します。クイズセットタブを選択し、クイズセット一覧が表示されている中から主催したいクイズセットを選択して、ID 列（一番左の列）部分を選択します。

![qdle_admin_ja_19.png](./doc/qdle_admin_ja_19.png)
...
![qdle_admin_ja_06.png](./doc/qdle_admin_ja_06.png)
  ...
  - **この画面をプロジェクターで投影するか、オンライン会議で画面共有してクイズを開催** します：
